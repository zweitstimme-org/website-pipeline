(function () {
  ...
  function viewForStep(s) {
    ...
    var land = {
      ...
      uncertainty: liftLandUncToPreamble(s.uncertainty || null, s.frac_reported),
      methodNote: null
    };
    return land;
  }
  ...
  /** Last preamble slot (HR, else Exit, else zs.org) — the CIs just left of 0 %. */
  function preambleUncFloorMap() {
    if (!isLivePage()) return null;
    var slots = preambleSlots();
    if (!slots.length) return null;
    var last = slots[slots.length - 1];
    var out = {};
    (last.rows || []).forEach(function (row) {
      PARTIES_ORDER.forEach(function (p) {
        var u = shareUnc(row, p);
        if (u > (out[p] || 0)) out[p] = u;
      });
    });
  ...
  /**
   * Night ribbon must not open thinner than the CIs immediately before 0 %.
   * Payload often stores the raw TV scrape (±0.5–0.8); preamble dots use the
   * lifted Exit/HR floor. Shrink that floor with the counted share.
   */
  function liftLandUncToPreamble(unc, frac) {
    if (!isLivePage() || !unc) return unc;
    var floorMap = preambleUncFloorMap();
    if (!floorMap) return unc;
    var f = Number(frac) || 0;
    if (f < 0) f = 0;
    if (f > 1) f = 1;
    var open = 1 - f;
    var next = {};
    var changed = false;
    Object.keys(unc).concat(Object.keys(floorMap)).forEach(function (p) {
      if (Object.prototype.hasOwnProperty.call(next, p)) return;
      var raw = Number(unc[p]) || 0;
      var fl = (Number(floorMap[p]) || 0) * open;
      next[p] = raw > fl ? raw : fl;
      if (next[p] !== raw) changed = true;
    });
    return changed ? next : unc;
  }
    ...
    el.innerHTML =
      ...
      '<div class="wb-wkr-hero">' +
        ...
        '<div class="wb-wkr-meta">' +
          '<dl>' +
          '<dt>P(Führung hält)</dt><dd>' + fmtProb(region.p_lead) + '</dd>' +
