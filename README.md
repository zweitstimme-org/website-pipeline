...
| `display_mode.json` | Whether to show forecast or Stimmung per scope; `archive.forecasts` lists runs after the 7-day post-election live window |
...
| `display_mode.R` | Election calendar, 90-day forecast window, live Vorhersagen for 7 days after election then archive |
...
| `FORECAST_ARCHIVE_DAYS` | `7` | Days after election to keep the last forecast in the live Vorhersagen block |
...
- `themes/PaperMod/layouts/partials/home_info_de.html` — dedicated **Vorhersage** section (active forecasts) and **Archiv – Vorhersagen** (post-election), included at Hugo build time from `data/display_mode.json` so empty blocks (and their background stripes) are omitted; Bundestag/Landtage sections show Kalman Stimmung only. Homepage banners live in `data/home_notices.json` + `layouts/partials/home_notices.html`, with a JS fallback (`static/js/home-notices.js`) loaded from `extend_head.html`. `scripts/ensure_home_notices.py` re-inserts those includes after every publish so Daily Stimmung cannot drop the banner. Frozen races pinned in `data/homepage_overrides.json` (and `display_mode.json` archive) are stripped from live Wahlkreise/Einzug tiles by `scripts/ensure_frozen_forecast_pages.py`.
